...
1i. **Память Empryo разобрана** — `.empryo/memory.db` (схема v6): таблица `memories` с категорией, `content_hash` для дедупа, `use_count`, два FTS5 (unicode61 и trigram), `memory_files` для привязки к файлам, `memory_edges` и `embedding`. `genome.db` — граф кода (files/symbols/edges/refs/cochanges), `-wal`/`-shm` это WAL-режим SQLite. Сессии остаются JSONL. titi берёт индекс без эмбеддингов: таблица `memories` рядом со `state.db`. Спека: `docs/research/empryo-port/storage.md`.
1j. **Индекс памяти с локальным эмбеддингом** — `titi-memory/src/index.rs`: `memory.db` рядом со `state.db`, дедуп по SHA-256, FTS5, привязка к файлам и эмбеддинг из хешированных триграмм (256d, без модели и сети). Recall ранжирует по косинусу + бонус за файл, который turn трогал, и вставляет только верхние 5 в системный промпт (`# Recalled`). Тул `memory` (remember/search, read-tier) отказывается хранить почти-дубль. Тест `recalled_memory_enters_the_system_prompt` — PASS.
1k. **Память больше не файл.** `identity_prompt` вставляет только SOUL и personality; `MEMORY.md`/`USER.md` в промпт не попадают. Индекс — единственная копия, которую видит модель. `memory.embeddingModel` в настройках выбирает эмбеддер: пусто или `local` — триграммный, иначе id модели через registry. Строка помнит, какой эмбеддер её посчитал, поэтому смена модели не сравнивает векторы из разных пространств. Кейсы и сравнение: `docs/research/empryo-port/memory-cases.md`.
1l. **Секреты маскируются до записи.** `titi-memory/src/redact.rs` заменяет ключи (`sk-`, `ghp_`, `AKIA`, PEM, JWT, `token=...`) на `[redacted]` и сообщает, сколько замаскировал. Хеш коммита и обычный текст про токены проходят. Тул `memory` получил `list` (все записи) и `models` (каталог `SUGGESTED_EMBEDDERS`: local, OpenAI small/large, Voyage, Cohere — для подсказок в настройках).
