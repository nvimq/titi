...
- [x] Очередь: во время стрима `Steer`/`FollowUp` ставятся в очередь, `Alt+Up` возвращает последнее в редактор, Esc снимает подсветку не удаляя (тест на `Composer::queue`).
