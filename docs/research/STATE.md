...
| 11. Mouse drag-select: Selection model, SGR-mouse decoding (Drag/ScrollUp/Down), selection background, SGR-mouse integration test, /mouse + --mouse пресеты, персист display.mouse_tracking | (current) | done, +9 unit (selection) + 3 input (Drag/Scroll) + 3 интеграционных (mouse_selection) + 3 App/config (titi-cli) |
...
titi-tui: 369 тестов (351 unit + 18 integration); titi-cli: 9 интеграционных; workspace: 566. Сборка 0 warnings; новые файлы clippy-clean.
...
3. ✅ Mouse drag-select: Selection model, SGR-mouse decoding, selection background, SGR-mouse integration test, `/mouse` + `--mouse` flag, персист display.mouse_tracking в конфиге (9 unit + 3 input + 3 интеграционных + 3 App/config теста).
